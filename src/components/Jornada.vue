<template>
  <section class="py-32 px-4 relative overflow-hidden" ref="sectionRef">
    ...
    <div class="max-w-6xl mx-auto relative z-10">
      ...
      <div class="relative max-w-[380px] mx-auto h-[650px] transition-all duration-1000 delay-300 transform" :class="[isVisible ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0', isVibrating ? 'animate-vibrate' : '']">
        ...
        <div class="absolute inset-0 bg-[#0A1128] rounded-[3rem] border-[8px] border-gray-900 shadow-2xl overflow-hidden flex flex-col relative">
          ...
          <!-- Header do Celular (Estilo WhatsApp Home) -->
          <div class="bg-[#111B3D] px-6 pt-10 pb-4 border-b border-gray-800 flex justify-between items-center shadow-md z-10">
          ...
          <!-- Área de Mensagens (Onde a Mágica Acontece - Lista de Conversas) -->
          <div class="flex-1 bg-[#0A1128] overflow-hidden relative">
            
            <div class="space-y-0 relative z-10 pt-2 h-full overflow-hidden flex flex-col justify-start">
              ...
              <div class="px-4 py-3 flex items-center gap-4 opacity-50 border-b border-gray-800/50 mt-auto">
  ...
  observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (!intervalId) {
          setTimeout(() => {
            if (isVisible.value) { // Check again in case they scrolled past quickly
